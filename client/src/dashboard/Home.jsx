...
function Home({ onLoad }) {
    const { user, token, members, switchPage, setToast, setShowInvite } = useDashboard();
    ...
    useEffect(() => {
        ...
        setLoading(false);
        if (onLoad) onLoad();
    }, [token, user]); // don't need to include fetchSessions/fetchTeam since they already redefine on token/user change :]
