...
function Team({ onLoad }) {
    const { user, token, setToast, setShowInvite, setShowPending } = useDashboard();
    ...
    useEffect(() => {
        ...
        setLoading(false);
        if (onLoad) onLoad();
    }, [token, user]); // don't need to include fetchTeam since it already redefines on token/user change
